...
public class MenuBanda {
    ...
    public boolean registrarDatosBanda(int seleccion) {
        ...
        int option = JOptionPane.showConfirmDialog(null, panel, mensaje, JOptionPane.OK_CANCEL_OPTION,
        ...
        if (option == JOptionPane.OK_OPTION) {
            ...
            while (!isValidDate(fechaCreacion, "dd/MM/yyyy")) {
                JOptionPane.showMessageDialog(null, "La fecha no esta en el formato correcto");
                option = JOptionPane.showConfirmDialog(null, panel, mensaje, JOptionPane.OK_CANCEL_OPTION,
                        JOptionPane.PLAIN_MESSAGE);
                if (option == JOptionPane.OK_OPTION) {
                    // Obtener los valores de los campos
                    genero = textFields[0].getText();
                    fechaCreacion = textFields[1].getText();
                }
            }
            switch (seleccion) {
                case 1:
